...
export async function POST(request: Request) {
    const { websiteId, userId, rating } = await request.json();
    ...
    try {
            ...
            [
                ...
                },
                {
                    $set: {
                        ratings: {
                            $cond: [
                                { $anyElementTrue: [{ $map: { input: '$ratings', as: 'r', in: { $eq: ['$$r.userId', userId] } } }] },
                                '$ratings',
        ...
        if (result && result.value) {
            return createCorsResponse({ success: true, newRating: result.value.evaluation });
        } else {
